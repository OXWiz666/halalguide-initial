...
// Query prayer facilities (usertype_id = 6)
$where_clause = "c.usertype_id = 6 AND c.status_id IN (1, 4)";
if (!empty($search)) {
...
<head>
    ...
    <title>Prayer Facilities - HalalGuide</title>
...
<body>
            ...
            <div class="nav-wrapper">
                ...
                <div class="nav-menu" id="navMenu">
                    <ul class="nav-links">
                        ...
                        <li><a href="prayer-facilities.php" class="nav-link active">Prayer Facilities</a></li>
                    </ul>
    ...
    <div class="page-header">
        <div class="container">
            <h1><i class="fas fa-mosque"></i> Prayer Facilities</h1>
            <p>Locate nearby mosques and prayer rooms wherever you travel</p>
            
            <!-- Search Bar -->
            <form method="GET" action="" class="search-bar">
                <input type="text" name="search" placeholder="Search prayer facilities..." value="<?php echo htmlspecialchars($search); ?>">
    ...
    <!-- Results -->
    <div class="container">
        <?php if (count($prayer_facilities) > 0): ?>
            <div class="results-count">
                Found <?php echo count($prayer_facilities); ?> prayer facility/facilities
            </div>
            
            <div class="establishments-grid">
                <?php foreach ($prayer_facilities as $facility): ?>
                    <?php
                    // Get company images
                    $company_images = [];
                    $company_image_storage = '../uploads/company/images/' . $facility['company_id'] . '/';
                    if (is_dir($company_image_storage)) {
                        ...
                        foreach ($img_files as $img_file) {
                            if ($img_file != '.' && $img_file != '..' && preg_match('/\.(jpg|jpeg|png|gif)$/i', $img_file)) {
                                $company_images[] = '../uploads/company/images/' . $facility['company_id'] . '/' . $img_file;
                            }
                        }
                    }
                    ...
                    <div class="establishment-card" data-aos="fade-up">
                        ...
                        <div class="establishment-content">
                            <div class="establishment-header">
                                <h3 class="establishment-name"><?php echo htmlspecialchars($facility['company_name']); ?></h3>
                                <div>
                                    <?php if ($facility['status_id'] == 4): ?>
                            ...
                            <p class="establishment-description">
                                <?php echo htmlspecialchars($facility['company_description'] ?: 'No description available'); ?>
                            </p>
                            
                            <?php if (!empty($facility['full_address'])): ?>
                                <div class="establishment-address">
                                    <i class="fas fa-map-marker-alt"></i>
                                    <span><?php echo htmlspecialchars($facility['full_address']); ?></span>
                                </div>
                            <?php endif; ?>
                            
                            <div class="establishment-contact">
                                <?php if ($facility['contant_no'] || $facility['tel_no']): ?>
                                    <div class="contact-item">
                                        <i class="fas fa-phone"></i>
                                        <span><?php echo htmlspecialchars($facility['contant_no'] ?: $facility['tel_no']); ?></span>
                                    </div>
                                <?php endif; ?>
                            ...
                            <div class="establishment-footer">
                                <a href="../pages/map/map.html?return=tourist&search=<?php echo urlencode($facility['company_name']); ?>" class="btn-view-map">
        ...
        <?php else: ?>
            <div class="no-results">
                <i class="fas fa-mosque"></i>
                <h3>No prayer facilities found</h3>
                <p><?php echo !empty($search) ? 'Try a different search term or browse all prayer facilities.' : 'No prayer facilities are available at the moment. Check back soon or register your prayer facility to get listed.'; ?></p>
                <?php if (empty($search)): ?>
